...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(f'{py_exec} -m pip install -r requirements.txt')
    system2(
        f'{py_exec} ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/rustdesk.exe')
    os.chdir('../..')
...
def main():
    ...
    py_exec = f'"{sys.executable}"'
    if not flutter:
        system2(f'{py_exec} res/inline-sciter.py')
    ...
    if windows:
        ...
        if os.path.exists('target/release/sciter.dll'):
            shutil.copy2('target/release/sciter.dll', os.path.join(res_dir, 'sciter.dll'))
        elif os.path.exists('sciter.dll'):
            shutil.copy2('sciter.dll', os.path.join(res_dir, 'sciter.dll'))
        if os.path.exists('target/release/deps/dylib_virtual_display.dll'):
            shutil.copy2('target/release/deps/dylib_virtual_display.dll', os.path.join(res_dir, 'dylib_virtual_display.dll'))
        elif os.path.exists('target/release/dylib_virtual_display.dll'):
            shutil.copy2('target/release/dylib_virtual_display.dll', os.path.join(res_dir, 'dylib_virtual_display.dll'))
        os.chdir('libs/portable')
        system2(f'{py_exec} -m pip install -r requirements.txt')
        system2(
            f'{py_exec} ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/RustDesk.exe')
        os.chdir('../..')
        if os.path.exists('./target/release/rustdesk-portable-packer.exe'):
            shutil.copy2('./target/release/rustdesk-portable-packer.exe', f'./rustdesk-{version}-install.exe')
            print(f'Installer created successfully: {os.path.abspath(f"./rustdesk-{version}-install.exe")}')
